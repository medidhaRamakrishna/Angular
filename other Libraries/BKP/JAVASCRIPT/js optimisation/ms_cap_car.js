...
F.PRIORITY.onChange(function () {
	if (F.DD_CURRENT_STAGE.read() == 'MGR') {
		...
		if (p1 != p2)
			if (F.APPROVE_CAR_INITIATION.read() == 1) {
				var resp1 = confirm(jsAlertMessages["PRIORITY_CHANGE"]);
				if (resp1) {
					clearOwnership();
					F.STP.deleteAllRows();
					F.STP.makeEmpty();
...
F.onLoad(function () {
	...
	if (F.USER_ACTION.read() == 2 || F.USER_ACTION.read() == 4 || F.USER_ACTION.read() == 7 || F.USER_ACTION.read() == 6 || F.USER_ACTION.read() == 13 || F.USER_ACTION.read() == 10) { //2-cancel,4-Seek additional info,7-change ownership,6-request for change,13transfer of ownership
...
F.CAR_OWNER.onChange(function () { //onChange of CAROwner Field
F.STEP_DESC.disableAll();
		...
		for (i = 1; i <= j; i++) {
			...
			if (i == 6)
				...
				if (F.STEP_OWNER.read(i) == '') {
					...
					//addinmain_lov(carOwnerStoredName, carOwnerFullName, getSeq(F.STEP_OWNER.sequence, i));
					//F.STEP_OWNER.write(carOwnerStoredName, i);
					//F.STEP_OWNER.write(F.CAR_OWNER.read(i), i);
					//F.STEP_OWNER.write(F.CAR_OWNER.read(i), i);
					//F.STEP_OWNER.updateGrid(i);
					//F.STEP_OWNER.writeLov(carOwnerFullName, 2);
					//alert("after"+F.STEP_OWNER.read(i));
					//F.STEP_OWNER.writeLov(carOwnerFullName, i);
		...
		for (var k = 0; k < tabsInfo.length; k++) {
			...
			if (str != 'Tabs.CONG_TEAM') {
				if (str == 'Tabs.ACTION') {
					...
					//var tabId = eval("(" + str + ")");
					//var tabId = eval(str);
					if (F.ACT.getRowCount() >= 1 && (F.ACTION_ITEM_TITLE.read() != '' && F.ACTION_RELATES_TO.read() != ''))
						F.getTab(tabId).show();
...
function removeFromAction() {
	...
	F.USER_ACTION.removeFromDropDown(4); //reject
...
function showActionTab() {
	...
	if (F.ACTION_ITEM_NO.read(row1) != '' && F.ACTION_ITEM_TITLE.read(row1) != '') {
...
F.USER_ACTION.onChange(function () {
	...
	if (F.USER_ACTION.readDisplayedValue == "Seek Additional Info" && F.DD_CURRENT_STAGE.read() == 'MGR') {
	...
	if (F.USER_ACTION.read() == 2 || F.USER_ACTION.read() == 4 || F.USER_ACTION.read() == 13) {
		if (F.DD_CURRENT_STAGE.read() == 'MGR')
...
function enableActionItems(q) {
	if (F.ACTION_RELATES_TO.read(q) == F.HID_STEP.read()) {
